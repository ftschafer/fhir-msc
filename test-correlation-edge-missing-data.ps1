...
.SYNOPSIS
    ...
    Creates 10 blocks where some are missing vital signs, NEWS2, or conditions.

.DESCRIPTION
    Posts FHIR data for 10 blocks (M01-M10) in neighborhood N94.
    Blocks M01-M06 have complete data; M07-M10 have gaps:
    ...
    Expected behaviour:
      - blockCount = 10 (all blocks have MeasureReports)
...
# 10 blocks: M01-M06 complete, M07-M10 have missing data
$blocks = @(
...
Write-Host "  EDGE CASE: Missing / Partial Data ($Neighborhood)" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
# ==========================================================================
# STEP 2 - MeasureReports (ALL 10 blocks - every block has demographic data)
# ==========================================================================
Write-Host ""
Write-Host "[2] Creating MeasureReports (10 blocks) ..."
$mrEntries = @()
foreach ($b in $blocks) {
    $mrEntries += @{
        resource = @{
...
# ==========================================================================
# STEP 3 - Patients (2 per block = 20)
# ==========================================================================
Write-Host ""
...
# ==========================================================================
# STEP 4 - Conditions (for blocks with cond > 0)
# ==========================================================================
Write-Host ""
...
# ==========================================================================
# STEP 5 - NEWS2 Observations (only for blocks with hasNews2=$true)
# ==========================================================================
Write-Host ""
$news2Blocks = $blocks | Where-Object { $_.hasNews2 }
Write-Host "[5] Creating NEWS2 Observations ($($news2Blocks.Count)) ..."
$news2Entries = @()
foreach ($b in $news2Blocks) {
    $news2Entries += @{
        resource = @{
...
# ==========================================================================
# STEP 6 - Vital Signs (only for blocks with hasVitals=$true)
# ==========================================================================
Write-Host ""
$vitalBlocks = $blocks | Where-Object { $_.hasVitals }
Write-Host "[6] Creating Vital Signs ($($vitalBlocks.Count * 3)) ..."
$vitalEntries = @()
foreach ($b in $vitalBlocks) {
    foreach ($vt in @(
...
try {
    ...
    Write-Host "  OK - blocks=$($corr.blockCount)" -ForegroundColor Green
} catch {
...
# blockCount = 10
Write-Host "  blockCount = 10 ... " -NoNewline
if ($corr.blockCount -eq 10) {
...
# averageIncome_vs_conditions: all 10 blocks have income + conditions counted
...
Write-Host "    samples = 10 (all blocks have demographic data) ... " -NoNewline
...
if ($corr.blockRows) {
    ...
    foreach ($row in $corr.blockRows | Sort-Object { $_.block }) {
        ...
        Write-Host ("  {0,-6} {1,10} {2,6} {3,6} {4} {5,5} {6} {7} {8}" -f $row.block,$row.averageIncome,$row.careUnits,$row.meanAge,$n2,$row.conditionCount,$hr,$sb,$db)
    }
}
...
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "  Design: 10 blocks, 4 with missing data"
