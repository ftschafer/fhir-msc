<#
.SYNOPSIS
    Kendall tau correlation test - INVERSE directions.
    Creates 20 blocks and validates Kendall tau B concordance coefficient
    with ALL directions flipped compared to the original test.

.DESCRIPTION
    Posts FHIR data for blocks I01-I20 in neighborhood N81, then validates
    that the Kendall tau B coefficient has the expected INVERSE direction,
    ...
    Data design (ordinal grouped, tie-rich - INVERSE directions):
      Same 5-tier ordinal structure as kendall-positive.
      care/cond now INCREASE across tiers (positive with income).
      HR/SBP/DBP now DECREASE across tiers (negative with age).
      Tied values within each 4-block tier exercise Kendall tau-B
      tie-correction sqrt((n0-n1)(n0-n2)).
#>
...
# ==========================================================================
# DATA - 20 blocks in 5 ordinal tiers of 4 blocks each - INVERSE directions
# Same tie structure as kendall-positive: care/cond tied within tiers.
# care/cond now increase, HR/SBP/DBP now decrease across tiers.
# ==========================================================================
$blocks = @(
    # Tier 1 - Deprived (lowest care, lowest income, youngest, high HR)
    @{ id="I01"; income=14000;  care=1;  age=28; news2=0.5;  cond=0;  hr=97;  sbp=171; dbp=97 }
    @{ id="I02"; income=15500;  care=1;  age=31; news2=0.8;  cond=1;  hr=96;  sbp=168; dbp=96 }
    @{ id="I03"; income=17000;  care=2;  age=34; news2=1.0;  cond=1;  hr=95;  sbp=166; dbp=95 }
    @{ id="I04"; income=19000;  care=2;  age=37; news2=1.4;  cond=2;  hr=94;  sbp=163; dbp=94 }
    # Tier 2 - Low-income
    @{ id="I05"; income=27000;  care=3;  age=44; news2=2.8;  cond=3;  hr=90;  sbp=158; dbp=90 }
    @{ id="I06"; income=30000;  care=3;  age=47; news2=3.5;  cond=3;  hr=89;  sbp=155; dbp=89 }
    @{ id="I07"; income=33000;  care=4;  age=50; news2=4.3;  cond=4;  hr=87;  sbp=152; dbp=87 }
    @{ id="I08"; income=37000;  care=4;  age=53; news2=5.2;  cond=4;  hr=85;  sbp=148; dbp=85 }
    # Tier 3 - Moderate-income
    @{ id="I09"; income=52000;  care=6;  age=59; news2=8.0;  cond=6;  hr=80;  sbp=141; dbp=80 }
    @{ id="I10"; income=58000;  care=6;  age=62; news2=9.5;  cond=6;  hr=78;  sbp=138; dbp=78 }
    @{ id="I11"; income=64000;  care=7;  age=65; news2=11.0; cond=7;  hr=76;  sbp=134; dbp=76 }
    @{ id="I12"; income=72000;  care=7;  age=68; news2=12.4; cond=7;  hr=74;  sbp=130; dbp=74 }
    # Tier 4 - Well-off
    @{ id="I13"; income=97000;  care=9;  age=72; news2=14.8; cond=9;  hr=70;  sbp=124; dbp=70 }
    @{ id="I14"; income=109000; care=9;  age=75; news2=16.0; cond=9;  hr=68;  sbp=120; dbp=68 }
    @{ id="I15"; income=122000; care=10; age=77; news2=17.0; cond=10; hr=66;  sbp=116; dbp=66 }
    @{ id="I16"; income=137000; care=10; age=80; news2=17.9; cond=10; hr=64;  sbp=112; dbp=64 }
    # Tier 5 - Affluent (highest care needs, highest income, oldest, low HR)
    @{ id="I17"; income=172000; care=12; age=82; news2=18.5; cond=12; hr=62;  sbp=108; dbp=62 }
    @{ id="I18"; income=194000; care=12; age=84; news2=19.0; cond=12; hr=61;  sbp=105; dbp=61 }
    @{ id="I19"; income=219000; care=13; age=87; news2=19.4; cond=13; hr=59;  sbp=102; dbp=59 }
    @{ id="I20"; income=247000; care=14; age=89; news2=19.7; cond=14; hr=57;  sbp=98;  dbp=57 }
)

# Expected INVERSE directions & minimum |coefficient| thresholds
$expected = @{
    "careUnits_vs_news2"          = @{ dir = "positive"; minR = 0.70 }
    "averageIncome_vs_conditions" = @{ dir = "positive"; minR = 0.70 }
    "meanAge_vs_heartRate"        = @{ dir = "negative"; minR = 0.80 }
    "meanAge_vs_systolicBP"       = @{ dir = "negative"; minR = 0.80 }
    "meanAge_vs_diastolicBP"      = @{ dir = "negative"; minR = 0.80 }
}
...
Write-Host "  $CoefficientName Test - INVERSE Directions ($Neighborhood)" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
...
$anchorBody = @{
    ...
    name = @(@{ family = "KendallNeg"; given = @("Anchor") })
}
    ...
    for ($p = 1; $p -le 3; $p++) {
        ...
        $patEntries += @{
            resource = @{
                ...
                name      = @(@{ family = "KndlNeg-$($b.id)"; given = @("P$p") })
...
# ==========================================================================
# STEP 10 - VALIDATE: Kendall tau (INVERSE directions)
# ==========================================================================
Write-Host ""
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host "  VALIDATION: $CoefficientName - INVERSE Directions" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
...
Write-Host ""
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host "  SPATIAL AUTOCORRELATION (MORAN I)" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
...
Write-Host ""
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host "  BLOCK DATA" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
if ($corr.blockRows) {
...
Write-Host ""
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host "  DIRECTION COMPARISON: Original vs Inverse" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
...
# ==========================================================================
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host "  SUMMARY - $CoefficientName, INVERSE Directions" -ForegroundColor DarkYellow
Write-Host "================================================================" -ForegroundColor DarkYellow
Write-Host ""
