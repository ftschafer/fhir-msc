 ...
 * neighbourhood-level MeasureReports.
 *
 * In the city branch the city extension is present on each block report, so
 * the neighbourhood is always extracted from the report itself (not from a
 * single configured value) — allowing multiple neighbourhoods per city.
 */
@Service
public class NeighborhoodMeasureReportAggregationService {
    ...
    public void aggregateNeighborhoodMeasureReports() {
        try {
            ...
            for (MeasureReport blockReport : blockReports) {
                // City branch: extract neighbourhood from the report extension itself
                // (supports multiple neighbourhoods per city)
                String neighborhood = normalize(extractNeighborhood(blockReport));
            ...
            List<MeasureReport> forwarded = new ArrayList<>();
    ...
    private boolean isBlockLayerReport(MeasureReport report) {
        ...
        for (Identifier identifier : report.getIdentifier()) {
            ...
            if (BLOCK_IDENTIFIER_SYSTEM.equals(identifier.getSystem())) return true;
        }
        return false;
    }
    ...
    private MeasureReport upsertNeighborhoodReport(IFhirResourceDao<MeasureReport> dao, NeighborhoodAccumulator acc) {
        if (acc.blockCount == 0) return null;
