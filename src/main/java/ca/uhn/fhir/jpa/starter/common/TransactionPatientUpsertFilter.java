...
/**
 * Makes transaction bundle patient writes idempotent when callers send
 * POST Patient with client-assigned IDs (which otherwise fails on retry).
 *
 * For each Bundle entry with:
 * - request.method = POST
 * - request.url = Patient
 * - resource.resourceType = Patient
 * - resource.id present
 *
 * this filter rewrites to:
 * - request.method = PUT
 * - request.url = Patient/{id}
 */
@Component
@Order(1)
public class TransactionPatientUpsertFilter extends OncePerRequestFilter {
    ...
    private RewriteResult maybeRewriteTransactionBundle(byte[] body) {
        try {
            ...
            if (!"Bundle".equals(rootObj.path("resourceType").asText())) {
                return null;
            }

            String bundleType = rootObj.path("type").asText();
            if (!("transaction".equalsIgnoreCase(bundleType) || "batch".equalsIgnoreCase(bundleType))) {
                return null;
            }
            ...
            ArrayNode entries = (ArrayNode) entriesNode;
            ...
            for (JsonNode entryNode : entries) {
                ...
                if (!(resourceNode instanceof ObjectNode) || !(requestNode instanceof ObjectNode)) {
                    continue;
                }
                ObjectNode resourceObj = (ObjectNode) resourceNode;
                ObjectNode reqObj = (ObjectNode) requestNode;
    ...
    private RewriteResult maybeRewriteSinglePatientCreate(HttpServletRequest request, byte[] body) {
        try {
            ...
            ObjectNode resourceObj = (ObjectNode) root;

            if (!"Patient".equals(resourceObj.path("resourceType").asText())) {
                return null;
            }
            ...
            if (requestUri == null || !requestUri.toLowerCase().endsWith("/fhir/patient")) {
                return null;
            }
