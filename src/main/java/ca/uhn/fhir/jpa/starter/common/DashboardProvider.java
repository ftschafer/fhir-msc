...
public class DashboardProvider implements IResourceProvider {
    ...
    private DashboardStats calculateStats(String filterBlock) {
        ...
        Map<String, VitalSignAccumulator> cityVitalMap = new HashMap<>();
        int totalPatients = 0;
        ...
        // Build city-level vital sign averages from neighborhood/block source values.
        for (Map.Entry<String, VitalSignAccumulator> vitalEntry : cityVitalMap.entrySet()) {
            VitalSignAverage avg = vitalEntry.getValue().toAverage(vitalEntry.getKey());
            if (avg != null) {
                stats.cityVitalSignAverages.add(avg);
            }
        }
    ...
    //  Statistical computation 
     ...
     */
    private StatsSummary computeStatsSummary(List<BlockStats> blocks, String cityScopeLabel) {
        ...
        s.neighborhood = cityScopeLabel;
        s.blockCount = withAgg.size();
        if (withAgg.isEmpty()) return s;

        // avg NEWS2 per block  from aggregateNews2, the same value the block cards display
        ...
        for (int i = 0; i < withAgg.size(); i++) {
            ...
            s.zScores.add(new BlockZScore(
                bs.block,
                bs.neighborhood != null ? bs.neighborhood : "",
    ...
    private void writeJsonResponse(HttpServletResponse response, DashboardStats stats) throws Exception {
        ...
        // City-level vital sign averages (derived from neighborhood/block source values)
        json.append("\"cityVitalSignAverages\":[");
        for (int i = 0; i < stats.cityVitalSignAverages.size(); i++) {
            VitalSignAverage vsa = stats.cityVitalSignAverages.get(i);
            if (i > 0) json.append(",");
            json.append("{");
        ...
        //  Statistical Summarization 
    ...
    static class DashboardStats {
        int totalPatients;
        int totalConditions;
        List<VitalSignAverage> cityVitalSignAverages = new ArrayList<>();
